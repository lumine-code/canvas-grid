...
class CanvasGrid {
  ...
  setWindow({
    ...
    const oldSelections = this.selections.map((selection) => ({
  ...
  bodyViewportWidth() {
    return Math.max(
      ...
      this.scrollElement.clientWidth ||
        (this.viewWidth || 0) - this.rowHeaderWidth,
    );
  }
  ...
  bodyViewportHeight() {
    return Math.max(
      ...
      this.scrollElement.clientHeight ||
        (this.viewHeight || 0) - this.headerHeight,
    );
  }

  logicalMaxScroll() {
    return Math.max(0, this.rowMetrics.totalSize - this.bodyViewportHeight());
  }

  physicalMaxScroll() {
    return Math.max(
      0,
      this.physicalHeight - this.headerHeight - this.bodyViewportHeight(),
    );
  }
  ...
  logicalScrollTop() {
    ...
    return (
      (this.scrollElement.scrollTop / physicalMax) * this.logicalMaxScroll()
    );
  }
  ...
  drawActiveOutline(ctx, scrollLeft, scrollTop) {
    ...
    ctx.rect(
      this.rowHeaderWidth,
      this.headerHeight,
      this.viewWidth - this.rowHeaderWidth,
      this.viewHeight - this.headerHeight,
    );
    ctx.clip();
    ctx.strokeStyle = this.colorAccent;
    ctx.lineWidth = 2;
  ...
  pointInScrollbar(clientX, clientY) {
    ...
    const clientWidth = this.scrollElement.clientWidth || rect.width;
    const clientHeight = this.scrollElement.clientHeight || rect.height;
    return (
      x >= this.scrollElement.clientLeft + clientWidth ||
      y >= this.scrollElement.clientTop + clientHeight
    );
  }
  ...
  hit(clientX, clientY, shouldClamp = false) {
    ...
    const contentX =
      Math.max(x, this.rowHeaderWidth) + this.scrollElement.scrollLeft;
  ...
  resizeHit(clientX, clientY) {
    ...
    if (this.pointInScrollbar(clientX, clientY)) return null;
    const rect = this.element.getBoundingClientRect();
    const x = clientX - rect.left;
    const y = clientY - rect.top;
    if (
    ...
    const contentX = x + this.scrollElement.scrollLeft;
  ...
  requestContextSort(direction, source) {
    const column =
      this.contextTarget?.zone === "column"
        ? this.contextTarget.column
        : this.activeCell().column;
  ...
  requestSort(column, direction = "cycle", source = "api") {
    if (
      ...
      !["ascending", "descending", "clear", "cycle"].includes(direction) ||
      column < 0 ||
