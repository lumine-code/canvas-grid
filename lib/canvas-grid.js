...
class CanvasGrid {
  ...
  drawRowHeaders(
    ...
    for (let rowIndex = firstRow; rowIndex <= lastRow; rowIndex++) {
      ...
      ctx.fillText(
        String(this.absoluteRow(rowIndex) + 1),
        CELL_PADDING_X,
        y + rowHeight / 2,
      );
    }
  ...
  setSelections(
    ...
    if (this.selection && active) {
      const cell = this.clampCell(active.row, active.column);
      this.selection.r1 = cell.row;
      this.selection.c1 = cell.column;
    }
    this.updateAria();
    if (announce) this.announceSelection();
    this.scheduleDraw("overlay");
  ...
  resizeHit(clientX, clientY) {
    ...
    if (
      ...
      const row = this.rowMetrics.indexAt(
        y - this.headerHeight + this.logicalScrollTop(),
      );
      const edge =
